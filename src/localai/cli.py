...
from localai.readiness import LIVE, READY, collect_product_status
...
def status(
    ...
    skip_inference: Annotated[
        ...
        typer.Option(
            "--skip-inference",
            help="Liveness only: never loads the model and never reports Ready.",
        ),
    ] = False,
) -> None:
    ...
    chat cannot work. With --skip-inference a healthy stack reports Live, not
    Ready: nothing proved the model can answer.
    """
    ...
    if json_output:
        ...
        typer.echo(f"==== AFK AI status ====  {snapshot.state}")
    ...
    raise typer.Exit(code=0 if snapshot.state in (READY, LIVE) else 1)
